...
function CultureDetail(){
    ...
    const {isLoading, isError, error, data}=useQuery(["culture-detail", cno], // data 라는 이름으로 받는다.
    
        async ()=> {
            return await apiClient.get(`/culture/detail/${cno}`)
        }
    )
    // 서버 연결 지연 상태
                ...
                <table className={"table"} style={{"borderCollapse": "collapse", "border": "none", "width": "70%"}}>
                    ...
                    <tr>
                        ...
                        <td colSpan={"2"} style={{"borderTop": "none"}}>
                            <h5><span style={{"color":"#fe5c24"}}>#&nbsp;{data.data.tag}&nbsp;&nbsp;# 역사 속 문화</span></h5>
                        </td>
                    </tr>
                    <tr>
                        <h3 style={{
                    ...
                    <tr>
                        ...
                        <td width={"55%"} style={{"borderTop": "none"}}>{data.data.content}</td>
                    </tr>
                    </tbody>
                </table>
                <hr style={{"width": "65%"}}/>

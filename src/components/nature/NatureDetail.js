...
function NatureDetail(){
    ...
    const {isLoading, isError, error, data}=useQuery(["nature-detail", nno], // data 라는 이름으로 받는다.
    
        async ()=> {
            return await apiClient.get(`/nature/detail/${nno}`)
        }
    )
    // 서버 연결 지연 상태
                ...
                <table className={"table"} style={{"borderCollapse": "collapse", "border": "none", "width": "70%"}}>
                    ...
                    <tr>
                        ...
                        <td colSpan={"2"} style={{"borderTop": "none"}}>
                            <h5><span style={{"color":"#fe5c24"}}>#&nbsp;{data.data.tag}&nbsp;&nbsp;# 자연 속 힐링</span></h5>
                        </td>
                    </tr>
                    <tr>
                        <h3 style={{
